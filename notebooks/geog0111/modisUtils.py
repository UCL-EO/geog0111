...
def NASAServerUp(zone="America/New_York",verbose=True,deadHour=9):
    '''
    test that it isn't Wednesday afternoon in Eastern USA
    which is when the NASA servers are down. We use NYC for
    the timezone (option zone). 
    
    Options:
        zone="America/New_York" : set the timezone for check
                                  see pytz.all_timezones for full list
        verbose=True
        deadHour=9              : the switchover time, so after this
                                  we assume the server not up
            
    
    Return: 
        True if it is *not* Wednesday 9am to Thursday 9am, so NASA servers likely up
        False if it *is* Wednesday 9am to Thursday 9am, so NASA servers likely *not* up
    '''
    newYorkTz = pytz.timezone(zone) 

    # weekday name for now
    d = datetime.datetime.now(newYorkTz)
    t = datetime.datetime.timetuple(d)

    if (d.date().strftime('%A') == 'Wednesday' and  t.tm_hour > deadHour) or \
       (d.date().strftime('%A') == 'Thursday' and  t.tm_hour <= deadHour):
        if verbose:
            try:
              cprint('\nWARNING: proceed with caution with tests on the NASA server today!' + \
                     f': it is {d.strftime("%A")} {d.strftime("%H")} Hrs' +
                     f' in {zone} and the NASA serveres are likely down for maintenance','red', attrs=['blink'])
            except:
              print('WARNING: proceed with caution with tests on the NASA server today!' + \
                     f': it is {d.strftime("%A")} {d.strftime("%H")} Hrs' + \
                     f' in {zone} and the NASA serveres are likely down for maintenance')
        return (False)
    else:
        if verbose:
            print('It is probably OK to proceed with tests on the NASA server today' + \
                 f': it is  {d.strftime("%A")} {d.strftime("%H")} Hrs' + \
...
def modisServer(product='MCD15A3H',version='061',**kwargs):
    ...
    or product[:5] == "VNP10" or product[:5] == "VNP10":
        # NSIDC
...
def modisURL(year=2020, month=1, day=1,tile='h08v06',\
    ...
    if html:
      ...
      # set the filename ext to hdf or h5 or whatever
      if product[:3] == 'VNP':
        filename_ext = 'h5' # VIIRS filename type ext
      else:
        filename_ext = 'hdf' # MODIS filename ext
      ...
      if len(filenames):
            ...
            if (verbose) :
              print(f'{filename_ext} file {filename}')  
      else:
            print(f"Problem with request: Can't find pattern {filename_start}*.{filename_ext} in {html_file}")
            #import pdb;pdb.set_trace()
            return None
...
def modisFile(year=2020, month=1, day=1,tile='h08v06',\
    ...
    # to cope with version decimals
    # eg 6.1 in the modis version field
    # sort version field
    if type(version) == int:
...
def get_platform_name(product):
    '''
    return the platform name (VIIRS, MODIS) as string
    or UNKNOWN

    '''
    if len(product) < 3:
      return 'UNKNOWN'
    if product[:3] == 'VNP' or product[:3] == 'VNJ':
      return 'VIIRS'
    if product[:3] == 'MCD' or product[:3] == 'MOD' or product[:3] == 'MYD':
      return 'MODIS' 
    return 'UNKNOWN'
...
def get_sds_name(name,product):
    '''
    get the sds from name, conditional on product
    product informs if VIIRS or MODIS
    sds is usually name.split()[1] with name e.g.

      [2400x2400] //HDFEOS/GRIDS/VNP_Grid_VNP15A2H/Data_Fields/Fpar 

    default, unknown, failure to read returns 'None' as a string
    '''
    # the default
    sds_name = 'None'
    ...
    try: 
      if get_platform_name(product) == 'VIIRS':
        # for VIIRS, you seem to get something like
        # //HDFEOS/GRIDS/VNP_Grid_VNP15A2H/Data_Fields/Lai
        # so we interpret via Path and take the name
        # so, LAI in this case
        sds_name = name.split()[1] 
        sds_name = Path(sds_name).name
        return sds_name 

      if get_platform_name(product) == 'MODIS':
        if product[:5] != "MCD64":
          sds_name = name.split()[1]
        else:
...
def getModisTiledata(doy=None,year=2020, month=1, day=1,tile='h08v06',\
    ...
    if g:
        for filename,name in g.GetSubDatasets():
            sds_name = get_sds_name(name,product)
            if (sds == None) or (sds == 'None') or (sds == ['None']) or (sds_name in sds):
                # get the SDS
                if verbose:
                    print(f'dataset info is: {name}')
                # read the dataset
