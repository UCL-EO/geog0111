...
```
    ...
    <matplotlib.legend.Legend at 0x7fb93aa56810>
...
```
    ...
    [<matplotlib.lines.Line2D at 0x7ff1b17eb290>]
...
```
    ...
    <matplotlib.legend.Legend at 0x7fb938f8bf10>
...
```
    ...
    <matplotlib.legend.Legend at 0x7fb938533c10>
...
* Create a Python script called `work/get_snow_cover.py`.
* This should include a function that when you run it for a given year, creates and stores a (gap-filled) measured snow cover dataset for that year, for each day of the year, averaged over over HUC catchment 13010001. 
* It should store the data in a CSV format file called `work/snow_cover_YYYY.csv` for year YYYY. 
* You should produce data files for both 2018 and 2019. 
...
You **can** the use a high-level function such as [`modis_annual`](geog0111/modis_annual.py) as previously to gather the dataset for the two years of interest, but note that if you do, the marks you get for that part of the code development will be limited to a pass grade. You can get more marks by developing, documenting and commenting your own codes for the MODIS processing. 

That said, the first time you go through the coursework, we recommend you *do* use a high-level function such as this or `Modis.get_modis()`, to make sure that you can complete this part of the coursework. If can later return and develop your own code, in which case you will have a previous result you can check yours against. You should *not* 

Since the function  [`modis_annual`](geog0111/modis_annual.py) is able to cache the dataset, you might like to run it the first time from a shell, with the verbose flag switched on. It may take some considerable time to stitch all of the data together, so being able to see the progress from the verbose output will be important (set the `verbose` flag when you set up the `Modis` class).
...
If, for any reason you have been able to produce these, discuss the matter with your course tutors before completing this section.
...
* Write a function called `calibrate()` to use a LUT approach to calibrate the 2-parameter snowmelt model presented above for a given year of data in a Python file `work/lut_model.py`. It should read the driving and calibration datasets from their CSV files, and output the model parameters and the goodness of fit metric at the LUT minimum, along with appropriate datasets that you can use to visualise and verify the calibration results. 
* Write a function called `validate()` in the Python file `work/lut_model.py` that reads the datasets for the validation year and the calibrated model parameters and validates the model. The main output will be the goodness of fit metric achieved in validation. It should output appropriate datasets that you can use to visualise the validation results. 
* When you run it as a script, it should run the calibration and validation functions and print a table of calibration and validation results.
* Submit `work/lut_model.py` as a PDF file [75% of marks for Model Inversion].

### Operation

* In a Jupyter notebook `work/lut_model.md`, load the function `calibrate()` from `work/lut_model.py`, run the model calibration for a given year of data and generate a table of the calibration parameters and associated statistics. 
* produce visualisations of the calibration results along with a paragraph of text describing what you are showing. 
* load the function `validate()` and use the other year of data to validate the model, i.e. run the model for the second year with the parameters you derived from the first (calibration) year. 
* produce visualisations of the validation results, along with a paragraph of text describing the validation results. 
* Submit  `work/lut_model.md` as a PDF file [25% of marks for Model Inversion].
