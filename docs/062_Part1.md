...
* Within this file `work/get_DelNorte_data.py` create a function called `main` that runs `write_DelNorte_data` for the years 2016 to 2019 inclusive. 
...
* In a Python notebook, produce plots for the Del Norte site of:
    ...
    1. Stream flow data for each year 2016 to 2019 inclusive (in units of megalitres/day - ML/day i.e. units of 1000000 litres a day), as a function of day of year.
    2. Maximum daily temperature in Celcius (C) for each year 2016 to 2019 inclusive as a function of day of year.
...
Whilst you *could* make use of wrapper codes we have written for plotting datasets, **we do not want you to do so**. Instead, we expect you to show us that you know how to use `matplotlib` to plot images and datasets. 
...
3. Code function `main` that runs get_DelNorte_data for the years 2016 to 2019 inclusive `[10%]`.
